...
(() => {
  ...
  function textWidth(value, fontSize, fontFamily = defaultFont, weight = 560) {
    ...
    if (document?.defaultView && "CanvasRenderingContext2D" in document.defaultView) {
      ...
      if (!context) {
        context = document.createElement("canvas").getContext("2d") ?? void 0;
  ...
  function separateSiblingBlocks(nodes, model, context) {
    ...
    for (const parent of [...parents, void 0]) {
      ...
      if (parent && !fixed(parent)) {
        resetContainerSize(parent, model);
  ...
  function compareRouteCost(left, right) {
    return left.crossings - right.crossings || left.branchCongestion - right.branchCongestion || left.bends + left.congestion / EDGE_CLEARANCE - (right.bends + right.congestion / EDGE_CLEARANCE) || left.length - right.length;
  }
  ...
  function routeCost(points, edges, independentEdges, branchEdges) {
    ...
    return { crossings, branchCongestion, bends: Math.max(0, route.length - 2), length, congestion };
  }
  ...
  function chooseSmartRoute(points, source, target, nodes, previousEdges, clearance = ROUTE_CLEARANCE, considerEdgeCrossings = true) {
    ...
    const initialCost = routeCost(initialRoute, comparedEdges, independentEdges, branchEdges);
    if (!pathBlocked(initialRoute) && initialCost.crossings === 0 && initialCost.congestion === 0 && initialCost.branchCongestion === 0) return points;
    ...
    const emptyCost = { crossings: 0, branchCongestion: 0, bends: 0, length: 0, congestion: 0 };
    const queue = [{
      xIndex: startX,
      yIndex: startY,
      direction: "start",
    ...
    while (queue.length && searchedStates < MAX_ROUTE_SEARCH_STATES) {
      ...
      for (const neighbor of neighbors) {
        ...
        const cost = {
          ...
          branchCongestion: current.cost.branchCongestion + segmentCongestion(from, to, branchEdges),
    ...
    if (!pathBlocked(initialRoute) && compareRouteCost(routeCost(simplified, comparedEdges, independentEdges, branchEdges), initialCost) >= 0) return points;
    return simplified;
  }
  function routeDeployment(nodes, model) {
    ...
    for (const plan of plans) {
      ...
      if (edge.from === edge.to) {
        ...
        const middleX = plan.sourceSide === plan.targetSide ? plan.sourceSide === "right" ? Math.max(start.x, end.x) + 24 : Math.min(start.x, end.x) - 24 : sourceBundle > 1 ? start.x + (plan.sourceSide === "right" ? 24 : -24) : (start.x + end.x) / 2;
        points = [start, { x: middleX, y: start.y }, { x: middleX, y: end.y }, end];
      } else if (!horizontalPort(plan.sourceSide) && !horizontalPort(plan.targetSide)) {
        const start = allocatedPorts.get(`${edge.id}:source`) ?? portPoint(source, plan.sourceSide, 0, 1);
        const end = allocatedPorts.get(`${edge.id}:target`) ?? portPoint(target, plan.targetSide, 0, 1);
        const sourceBundle = ["er", "slide"].includes(model.kind) ? 0 : requestsByPort.get(`${source.id}:${plan.sourceSide}:source`)?.length ?? 0;
        const middleY = plan.sourceSide === plan.targetSide ? plan.sourceSide === "bottom" ? Math.max(start.y, end.y) + 24 : Math.min(start.y, end.y) - 24 : sourceBundle > 1 ? start.y + (plan.sourceSide === "bottom" ? 24 : -24) : (start.y + end.y) / 2;
  ...
  var sequenceLayout = {
    ...
    layout(model, context) {
      ...
      const messageLabelHeight = (edge) => {
        const source = map.get(edge.from);
        const target = map.get(edge.to);
        const available = source && target ? Math.max(48, Math.abs(center(source).x - center(target).x) - 24) : 180;
      ...
      const edges = model.connections.flatMap((edge, index) => {
        ...
        messageY += Math.max(messageGap, (nextMessage ? messageLabelHeight(nextMessage) : 0) + 24 + (fromX === toX ? 28 : 0));
      ...
      const groups = model.groups.map((group) => {
        ...
        const top = (groupedEdges[0]?.points[0]?.y ?? firstMessageY) - headerHeight - (groupedEdges[0] ? messageLabelHeight(groupedEdges[0]) : 0) - 12;
  ...
  function routeActivity(nodes, model) {
    if (model.kind === "activity") return routeDeployment(nodes, model);
    const map = byId(nodes);
    const routedEdges = [];
    for (const edge of model.connections) {
      const source = map.get(edge.from);
  ...
  function placeEdgeLabels(geometry, fontSize, measure = (text) => estimatedTextWidth(text, fontSize)) {
    ...
    for (const edge of geometry.edges) {
      ...
      for (let i = 1; i < route.length; i += 1) {
        ...
        if (!length) continue;
        const horizontal = from.y === to.y;
        const lines2 = wrappedLines(edge.label, horizontal ? Math.min(maximumWidth, Math.max(48, length - 24)) : maximumWidth, measure);
        ...
        for (const fraction of branch ? [near, 0.5, 1 - near] : [0.5, near, 1 - near]) {
          ...
          for (const side of [-1, 1]) {
            const point = horizontal ? { x, y: y + (side < 0 ? -8 - extraHeight : fontSize + 8) } : { x: x + side * (width / 2 + 10), y: y + fontSize * 0.35 - extraHeight / 2 };
            candidates.push({
              placement: { ...textBounds(point, width, fontSize, extraHeight), point, lines: lines2, lineHeight },
              preference: Math.max(0, needed - length) * 4 + (branch ? (traveled + length * fraction) * 0.15 : horizontal ? 0 : 30) + Math.abs(fraction - 0.5) * (branch ? 0 : 10) + (side > 0 ? 1 : 0) + lines2.length * 2
            });
          }
        }
        traveled += length;
      ...
      const lines = wrappedLines(edge.label, maximumWidth, measure);
  ...
  function addLabel(group, context, yOffset = 0, paddingX = context.theme.nodePaddingX, maximum = MAX_NODE_LABEL_WIDTH) {
  ...
  var containerShape = {
    ...
    measure: ({ label, theme, attributes }) => {
      ...
      const headerHeight = 42 + (text.lines.length - 1) * text.lineHeight;
      return { width: Math.max(168, text.width + 42), height: Math.max(110, headerHeight + 26), headerHeight };
    },
    render(context) {
      const { node, theme, document } = context;
      const group = groupFor(context);
      ...
      setTextLines(text, node.label, Math.min(node.width - 32, wrapWidth(node.attributes, 216)), theme.fontSize - 1, theme.fontFamily, 650);
      group.append(text);
      return group;
    }
  };
  ...
  var componentShape = {
    ...
    measure: ({ label, theme, attributes }) => {
      const size = baseSize(label, { theme, attributes });
      return { width: Math.max(126, size.width), height: size.height + 26 };
    },
    render(context) {
      const { node, theme, document } = context;
  ...
  function slideExtra(context, value, width, fontSize, lineHeight, weight = 750) {
  ...
  function growSlideSize(name, context, size) {
    ...
    const w = size.width;
    const minimum = name === "slide-card" && attributes.badge ? attributes.body ? 110 : 76 : name === "slide-card" && attributes.body ? 90 : name === "slide-note" && attributes.body ? 72 : name === "slide-callout" && attributes.body ? 90 : 0;
    let growth = 0;
    switch (name) {
      case "slide-title":
  ...
  var slideCalloutShape = {
    ...
    measure: (context) => {
      const { attributes } = context;
      return growSlideSize("slide-callout", context, { width: numericAttribute(attributes, "width", 520, 240, 1e3), height: numericAttribute(attributes, "height", attributes.body ? 112 : 82, 70, 240) });
    },
    render(context) {
      const { node, theme, document } = context;
      const group = groupFor(context);
  ...
  var slideMilestoneShape = {
    ...
    render(context) {
      ...
      group.append(dot);
      if (node.attributes.period) appendSlideText(group, context, node.attributes.period.toUpperCase(), { x: 50, y: 29, width: node.width - 68, fontSize: 10, lineHeight: 12, weight: 760, color: theme.accentColor });
      const periodExtra = slideExtra(context, node.attributes.period?.toUpperCase() ?? "", node.width - 68, 10, 12, 760);
      appendSlideText(group, context, node.label, { x: 50, y: (node.attributes.period ? 54 : 43) + periodExtra, width: node.width - 68, fontSize: 16, lineHeight: 20, weight: 720 });
      if (node.attributes.body) appendSlideText(group, context, node.attributes.body, { x: 32, y: 88 + periodExtra + slideExtra(context, node.label, node.width - 68, 16, 20, 720), width: node.width - 50, fontSize: 12, lineHeight: 17, color: theme.mutedColor });
      return group;
    }
  };
  var slideGroupShape = {
  ...
  var umlClassShape = {
    ...
    measure: ({ label, attributes, theme }) => {
      ...
      const title = labelLayout(label, wrapWidth(attributes, 216), theme.fontSize + 1, theme.fontFamily, 720);
      const longest = Math.max(title.width, ...members.map((member) => textWidth(member.text, theme.fontSize - 1)));
    ...
    render(context) {
      ...
      const titleLayout = labelLayout(node.label, Math.min(node.width - 34, wrapWidth(node.attributes, 216)), theme.fontSize + 1, theme.fontFamily, 720);
      ...
      setTextLines(title, node.label, Math.min(node.width - 34, wrapWidth(node.attributes, 216)), theme.fontSize + 1, theme.fontFamily, 720);
  ...
  var usecaseShape = {
    ...
    measure: ({ label, theme, attributes }) => {
      ...
      return { width: Math.max(150, Math.ceil((text.width + 36) * Math.SQRT2)), height: Math.max(72, Math.ceil((text.height + 24) * Math.SQRT2)) };
    },
    render(context) {
      const { node, theme, document } = context;
      const group = groupFor(context);
  ...
  var umlRequiredInterfaceShape = {
    ...
    measure: ({ label, theme, attributes }) => {
      ...
      return { width: Math.max(76, text.width + 20), height: 30 + text.height + 8 };
    },
    render(context) {
      const { node, theme, document } = context;
      const group = groupFor(context);
