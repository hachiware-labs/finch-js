...
function sourceExpression(expression, values, evaluate2 = true, invoke) {
  const tokens = [];
  ...
  const result = or(evaluate2);
  if (index !== tokens.length) throw new Error("Unexpected source condition token.");
  return result;
}
function sourceCondition(expression, values, evaluate2 = true, invoke) {
  return truth(sourceExpression(expression, values, evaluate2, invoke));
}
...
function preprocess(source, options = {}) {
  ...
  const expand = (input, stack) => {
    ...
    const evaluate2 = (expression, enabled2 = true) => sourceExpression(expression, values, enabled2, (name, args) => invoke(name, args, stack));
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      if (loop) {
        ...
        try {
          if (loop[1] === "while") {
            ...
            let items;
            const listValue = list.startsWith("[") ? list : evaluate2(list);
      ...
      if (returned) {
        ...
        const value = evaluate2(returned[1]);
      ...
      if (assignment) {
        const value = evaluate2(assignment[2]);
...
// src/paired-block-order.ts
var mode = "full";
var stats = {};
function configure(value) {
  mode = value;
  stats = { evaluations: 0, pairs: 0 };
}
function orderConnectedBlocks(nodes, model, route) {
  if (mode === "off") return;
  const started = performance.now();
  if (!["graph", "deployment"].includes(model.kind) || nodes.length > 30 || model.connections.length > 40) return;
  if (model.connections.some((e) => e.attributes?.fromPort || e.attributes?.toPort || e.attributes?.layoutDirection || ["routing", "crossingCost", "bendCost", "lengthCost", "crossingClearance"].some((k) => e.attributes?.[k] !== void 0))) return;
  const locked = (n) => ["order", "row", "column", "place"].some((k) => n.attributes[k] !== void 0);
  const groups = [];
  ...
  const perspectives = (group) => {
    const patterns = [];
    const keys = [...new Set(model.connections.map((e) => e.label?.trim()).filter(Boolean))];
    for (const key of keys) {
      const ids = model.connections.filter((e) => e.label?.trim() === key).flatMap((e) => [e.from, e.to]);
      ...
      const existing = patterns.find((p) => p.nodes.map((n) => n.id).join(",") === ordered.map((n) => n.id).join(","));
      if (existing) existing.keys.push(key);
      else if (patterns.length < 3) patterns.push({ keys: [key], nodes: ordered });
    }
    if (!patterns.length) patterns.push({ keys: ["declaration"], nodes: group });
    return patterns;
  };
  const permutations = (xs) => xs.length ? xs.flatMap((n, i) => permutations(xs.filter((_, j) => j !== i)).map((rest) => [n, ...rest])) : [[]];
  const score = (edges) => {
    let shared = 0, crossings = 0, bends = 0, length = 0, reverse = 0;
    const segments = edges.flatMap((e, index) => {
      const source = nodes.find((n) => n.id === e.from), target = nodes.find((n) => n.id === e.to);
      if (source && target) for (let j = 1; j < e.points.length; j++) {
        const a = e.points[j - 1], b = e.points[j];
        const vertical = model.direction === "down";
        const delta = vertical ? b.y - a.y : b.x - a.x;
        const forward = vertical ? target.y - source.y : target.x - source.x;
        reverse += Math.max(0, -delta * Math.sign(forward));
      }
      bends += Math.max(0, e.points.length - 2);
      return e.points.slice(1).map((b, i) => {
        const a = e.points[i];
    ...
    for (let i = 0; i < segments.length; i++) for (const b of segments.slice(i + 1)) {
      ...
      const overlap = (a2, b2, c, d) => Math.max(0, Math.min(Math.max(a2, b2), Math.max(c, d)) - Math.max(Math.min(a2, b2), Math.min(c, d)));
    ...
    return [0, shared * 6 + reverse * 12 + crossings * 120 + bends * 48 + length];
  };
  const better = (a, b) => a[0] < b[0] - 0.01 || Math.abs(a[0] - b[0]) <= 0.01 && a[1] < b[1] - 0.01;
  let pairs = 0;
  ...
  for (let i = 0; i < groups.length; i++) for (const right of groups.slice(i + 1)) {
    ...
    const slots = (g) => g.map((n) => ({ x: n.x + n.width / 2, y: n.y + n.height / 2 }));
    const aSlots = slots(left), bSlots = slots(right);
    if (model.kind === "graph" && left.length === 3 && right.length === 3 && !nodes.some((n) => n.parentId)) {
      const axis = model.direction === "down" ? "y" : "x";
      for (const positions of [aSlots, bSlots]) {
        const center2 = [...positions].sort((a, b) => a[axis] - b[axis])[1][axis];
        positions.forEach((p) => p[axis] = center2);
      }
    }
    const variants = model.kind === "graph" && left.length === 3 && right.length === 3 && !nodes.some((n) => n.parentId) ? [[0, false], [1, false], [1, true]] : [[0, false], [0, true]];
    ...
    stats.pairs += orderPairs.length;
    stats.patterns = [lp.length, rp.length];
    for (const { a, b } of orderPairs) for (const [stagger, ports] of variants) {
      nodes.forEach((n, j) => Object.assign(n, original[j]));
      for (const [g, positions] of [[a, aSlots], [b, bSlots]]) g.forEach((n, j) => {
        n.x = positions[j].x - n.width / 2;
        n.y = positions[j].y - n.height / 2;
      });
      if (stagger) {
        const vertical = model.direction === "down";
        const shift = (n, d) => {
        ...
        shift(b[Math.floor(b.length / 2)], stagger === 1 ? -48 : 48);
        if (stagger === 1) shift(a[a.length - 1], 32);
      }
      const candidateModel = ports ? { ...selectedModel, connections: selectedModel.connections.map((e) => {
        const from = nodes.find((n) => n.id === e.from), to = nodes.find((n) => n.id === e.to);
        if (!from || !to) return e;
        const vertical = model.direction === "down";
        const forward = vertical ? to.y > from.y : to.x > from.x;
        return { ...e, attributes: { ...e.attributes, fromPort: vertical ? forward ? "bottom" : "top" : forward ? "right" : "left", toPort: vertical ? forward ? "top" : "bottom" : forward ? "left" : "right" } };
      }) } : selectedModel;
      ...
      if (invalid) continue;
      stats.evaluations++;
      const candidate = score(route(candidateModel));
  ...
  stats.searchMs = performance.now() - started;
  return selectedModel;
}
function evaluate(nodes, model, edges) {
  const score = (edges2) => {
    let shared = 0, crossings = 0, bends = 0, length = 0, reverse = 0;
    const segments = edges2.flatMap((e, index) => {
      const source = nodes.find((n) => n.id === e.from), target = nodes.find((n) => n.id === e.to);
      if (source && target) for (let j = 1; j < e.points.length; j++) {
        const a = e.points[j - 1], b = e.points[j];
        const vertical = model.direction === "down";
...
function arrangeBipartiteBlocks(nodes, model) {
  if (model.kind !== "graph" || nodes.length > 24 || nodes.some((n) => n.parentId || n.shape === "container")) return;
  if (nodes.some((n) => ["order", "row", "column", "place", "layout"].some((k) => n.attributes[k] !== void 0))) return;
  ...
  const crossSize = (n) => horizontal ? n.height : n.width;
  const length = (set) => set.reduce((sum, n) => sum + crossSize(n), 0) + (set.length - 1) * gap;
  ...
  for (const [index, set] of [sources, targets].entries()) {
    ...
    for (const n of set) {
      if (horizontal) {
        n.x = 28 + index * (depth + corridor);
        n.y = offset;
      } else {
        n.x = offset;
        n.y = 28 + index * (depth + corridor);
      }
      offset += crossSize(n) + gap;
    }
  }
}
...
function routingPolicy(attributes = {}) {
  const mode2 = attributes.routing ?? "balanced";
  if (mode2 !== "balanced" && mode2 !== "avoid-crossings") throw new Error(`Unknown routing policy: ${mode2}`);
  ...
  return { mode: mode2, crossing: value("crossingCost", 120), bend: value("bendCost", 48), length: value("lengthCost", 1), crossingClearance: value("crossingClearance", 18) };
}
...
function arrangeRetryBlocks(nodes, model, ranks2) {
  if (model.kind !== "flowchart" || model.direction !== "down" || nodes.some((n) => n.parentId)) return;
  if (nodes.some((n) => ["order", "row", "column", "place", "layout"].some((k) => n.attributes[k] !== void 0))) return;
  if (model.connections.some((e) => e.attributes?.fromPort || e.attributes?.toPort || e.attributes?.layoutDirection)) return;
  const spine = mainPath(model, ranks2);
  ...
  for (const node of nodes) {
    ...
    const decision = byId2.get(incoming[0].from), target = byId2.get(outgoing[0].to);
    if (!decision || !target || decision.shape !== "diamond" || !spine.has(decision.id) || !spine.has(target.id)) continue;
    if ((ranks2.get(target.id) ?? Infinity) >= (ranks2.get(decision.id) ?? -Infinity)) continue;
    ...
    const x = left ? Math.min(node.x, decision.x - gap - node.width, target.x - gap - node.width) : Math.max(node.x, decision.x + decision.width + gap, target.x + target.width + gap);
...
var baseHierarchicalLayout = {
  ...
  layout(model, context2) {
    ...
    if (!context2.overlay.frozen && !Object.keys(context2.overlay.nodes).length) arrangeBipartiteBlocks(nodes, model);
...
function layoutVerticalDirected(model, context2, includeContainers = false) {
  ...
  if (!context2.overlay.frozen && !Object.keys(context2.overlay.nodes).length) arrangeRetryBlocks(nodes, model, rankById);
  arrangeStateAxis(nodes, model);
  arrangeStateCycles(nodes, model);
  let orderedModel;
  if (!context2.overlay.frozen && !Object.keys(context2.overlay.nodes).length) {
...
var SvgRenderer = class {
  ...
  containerHeadingElements = /* @__PURE__ */ new Map();
  edgeElements = /* @__PURE__ */ new Map();
  edgeLabelElements = /* @__PURE__ */ new Map();
  labelPlacements = /* @__PURE__ */ new Map();
...
var DiagramInstance = class {
  ...
  zoomMode = "auto";
  resizeObserver;
  containerMinimumSizes = /* @__PURE__ */ new Map();
  destroyed = false;
